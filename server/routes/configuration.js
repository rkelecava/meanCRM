...
router.post('/adminInfo', function (req, res, next) {
	var adminInfo = new AdminInfo(req.body);

	adminInfo.save(function (err, adminInfo) {
		if (err) { return next(err); }
		res.json(adminInfo);
	});
});

router.param('adminInfo', function (req, res, next, id) {

	// Define a new query to search for setup by ID
	var query = AdminInfo.findById(id);

	// Execute the query
	query.exec(function (err, adminInfo) {
		// Return any errors
		if (err) { return next(err); }
		// If no setup is found with given ID, return error
		if (!adminInfo) { return next(new Error('can\'t find any admin information')); }
...
router.get('/adminInfo/:adminInfo', function (req, res, next) {

	// Return the setup from the route parameter
	return res.json(req.adminInfo);
});

router.delete('/adminInfo/:adminInfo', function (req, res, next) {

	// Remove setup using mongoose remove method
	req.adminInfo.remove(function (err, adminInfo) {
		// Return any errors
		if (err) { return next(err); }
...
router.put('/adminInfo/:adminInfo', function (req, res, next) {

	AdminInfo.findById(req.adminInfo._id, function (err, adminInfo) {

		if (req.body.first) { adminInfo.first = req.body.first; }
		if (req.body.last) { adminInfo.last = req.body.last; }
		if (req.body.title) { adminInfo.title = req.body.title; }
		if (req.body.email) { adminInfo.email = req.body.email; }
		if (req.body.phone) { adminInfo.phone = req.body.phone; }
